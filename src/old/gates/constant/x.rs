...
/// The one-qudit X (shift) gate. This is a Clifford/Weyl-Heisenberg gate.
///
/// The qubit (radix = 2) X gate is given by the following matrix:
///
/// $$
/// \begin{pmatrix}
///     0 & 1 \\\\
...
/// $$
///
/// The qutrit (radix = 3) X gate is given by the following matrix:
///
/// $$
/// \begin{pmatrix}
///     0 & 0 & 1 \\\\
...
impl<C: ComplexScalar> UnitaryFn<C> for XGate {
    #[inline]
    fn write_unitary(&self, _params: &[C::Re], utry: &mut MatMut<C>) {
        for i in 0..self.radix {
            utry.write((i + 1) % self.radix, i, C::one());
            utry.write(i, i, C::zero());
        }
    }
}
...
mod test {
    ...
    #[test]
    fn test_qubit_x_gate() {
        let gate = XGate::new(2);
